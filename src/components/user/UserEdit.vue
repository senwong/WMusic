<template>
  <div class="user-edit__container">
    ...
    <div class="user-edit__save-canel-btn">
      <Button class="user-edit__save-btn" primary @click.native="handleSave" :disabled="isNotChange || isSaving">保存</Button>
...
export default {
  ...
  computed: {
    ...
    isNotChange() {
      ...
      return this.nicknameTemp == nickname
    ...
    cityOptions() {
      ...
      return province && province.children ? province.children.map(c => ({
        key: c.code,
        value: c.code,
        title: c.name,
      })) : [];
